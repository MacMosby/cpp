...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	std::cout << "Trap " << this->_name << " is damaged and has ";
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (this->_energy_points > 0 && this->_hit_points > 0)
	{
		this->_energy_points--;
		this->_hit_points += amount;
		std::cout << "Trap " << this->_name << " repairs itself and regains ";
		std::cout << amount << " hit points to have now: " << this->_hit_points << std::endl;
		std::cout << "It has " << this->_energy_points << " energy points left" << std::endl;
	}
	else
	{
		std::cout << "Trap " << this->_name << " is missing hit or energy points";
